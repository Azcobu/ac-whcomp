...
def get_ac_rlt_items(npc_id, item_qual, db, cursor):
    itemdict = {}
    query = ('SELECT ilt.reference, ilt.chance, ilt.MaxCount '
             'FROM `item_loot_template` ilt '
             'JOIN `item_template` it ON it.entry = ilt.item '
             f'WHERE it.entry = {npc_id} AND ilt.reference != 0')
...
def get_npc_name(npc_id, cursor):
    query = f'SELECT it.name FROM `item_template` it WHERE it.entry = {npc_id}'
    cursor.execute(query)
...
def get_ac_items(npc_id, item_qual):
    ...
    query = ('SELECT it.entry, it.name, it.itemlevel, ilt.chance, it.quality '
             'FROM `item_loot_template` ilt '
             'JOIN `item_template` it ON it.entry = ilt.item '
             f'WHERE ilt.entry = {npc_id} AND ilt.reference = 0')
    if item_qual:
        query += f' AND it.quality >= {item_qual}'
...
def get_wh_items(npc_id, item_qual):
    url = f'https://tbc.wowhead.com/item={npc_id}'
    try:
